...
from catus.services.utils.query_parser import parse
...
class BaseService(object):
    ...
    def _render_row_value(self, row_data, render):
        ...
        if isinstance(render, basestring):
            if isinstance(row_data, dict):
                return row_data[render]
            else:
                return getattr(row_data, render)
        else:
            return render(row_data)
    ...
    def open_search(self, request, columnIndexNameMap, columnSortIndexNameMap, model, querySet=None, search=True, extra_response_params={}, *args, **kwargs):
        ...
        if "order" in open_search_data and open_search_data["order"]:
            sort_col = open_search_data["order"][0]
            if columnSortIndexNameMap.get(int(sort_col["column"])):
                sort_prop_name = columnSortIndexNameMap[int(sort_col["column"])]
                if sort_col["dir"] == "desc":
                    sort_prop_name = "-%s" % sort_prop_name

        if querySet is None:
            if sort_prop_name:
                querySet = model.objects.filter(*args, **kwargs).order_by(sort_prop_name)
            else:
                querySet = model.objects.filter(*args, **kwargs)
        else:
            if sort_prop_name:
                querySet = querySet.order_by(sort_prop_name)
